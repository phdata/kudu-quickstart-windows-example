...
The `launch_kudu_powershell` script usage for bringing up Kudu is as follows: 
 
```launch_kudu_powershell -kudu_dir [git location for kudu]```
...
You can create a schema manually for this data, but a general purpose function for setting specific columns as not nullable in the DataFrame is used instead.
```
...
The DataFrame will be instantiated, but the PK columns (agency, lab_name, facility) are nullable. This won't work with Kudu because PK columns cannot be null.  Use the `setNotNull` method to prepare the DataFrame for writing to Kudu.  
